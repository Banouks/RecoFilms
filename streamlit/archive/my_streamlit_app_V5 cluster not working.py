...
st.write(df_films[df_films['cluster'] == 1].sort_values(by= 'popularity', ascending= False))
...
def recherche_film(des_mots, un_df) :
        ...
        des_mots =  remplacer(des_mots) #remplacmeent des lettres accentuées
...
def recherche_casting(un_id, un_df):
    ...
    # Limitation aux 3 1ere personnes
    if len(liste) > 6 :
        liste = liste[:6]
    return liste
...
def mlKNN(un_df):
    ...
    X = un_df[['original_language', 'startYear2', 'firstGenre', 'secondGenre', \
            'averageRating2', 'numVotes2', 'popularity2']]
...
if recherche : 
    ...
    if choix_film :
        ...
        tconst_film 
        df_films[df_films['tconst'] == 'tt1877830']
        cluster = df_films[df_films['tconst'] == tconst_film[0]]['cluster']
        cluster.iloc[0]
        df_cluster = df_films[df_films['cluster'] == cluster.iloc[0]]
        df_cluster[df_cluster['tconst'] == 'tt1877830']
        filmKNN = mlKNN(df_cluster)
        # Je fais ma recherche de voisin et j'affiche les résultats (retourne les index)
        ideeFilm = filmKNN.kneighbors(parametre_film(tconst_film, df_cluster))
        ideeFilm
        ...
        with col1 :
            ...
            if df_films[df_films['tconst'] == tconst_film[0]]['poster_path'].item() != 'None' : 
                image_film = load_image_from_github(path_image + df_films[df_films['tconst'] == tconst_film[0]]['poster_path'].item())
            else : 
                image_film = Image.open(chemin + '/image-non-disponible.png')
            st.image(image_film, width= 200)
            st.write(df_films[df_films['tconst'] == tconst_film[0]]['originalTitle'].item())
        with col2 :
            st.subheader("Note : " + str(df_films[df_films['tconst'] == tconst_film[0]]['averageRating'].item()))
        ...
        with col3 :
            ...
            st.write(str(df_tmdb[df_tmdb['tconst'] == tconst_film[0]]['overview'].item()))
